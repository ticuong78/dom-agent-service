...
export class OverridenSHA256HashAdapter implements SHA256HashAdapter {
  /**
...
export type DiffViewerType =
  | "hierarchy"
  | "mutation"
  | "subshape"
  | "composite";

export const getDiffViewer = (type: DiffViewerType) => {
  switch (type) {
    case "hierarchy":
...
export const getContextTree = (html: string) => {
  ...
  const converter = new HTMLToContextConverter(
    ...
    new OverridenSHA256HashAdapter(),
  );
